#!/usr/bin/env python3
"""Aggregate Phase 3 confirmatory fairness runs for paper-facing Phase 4.

The Phase 3 launcher intentionally writes seed-isolated result trees. This
script joins those trees into a single ledger for paper tables and performs the
sanity checks described in CONFIRMATORY_RERUN_INSTRUCTIONS.md:
...
def collect_fairsmote_rows(root: Path) -> list[Row]:
    """Collect add-on Fair-SMOTE rows for the paired-bias experiments."""
...
def write_report(
    ...
    report.append("- `confirmatory_primary_summary.csv`: paper-facing behavior and mechanism metrics.")
...
- Phase-4 output root: `{args.output}`
...
- `phase4/PHASE4_CONFIRMATORY_REPORT.md`
...
- `phase4/scott_knott_esd_groups.csv` (paper-facing standard `ScottKnottESD` grouping)
- `phase4/SCOTT_KNOTT_ESD_VALIDATION_REPORT.md`
- `phase4/scott_knott_esd_exclusions.csv`
- `phase4/scott_knott_esd_comparison.csv`
- `phase4/scott_knott_groups.csv` (lightweight diagnostic grouping; not the paper-facing statistical result)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional Fair-SMOTE add-on result root. When provided, Fair-SMOTE "
    ...
    write_report(
        args.output / "PHASE4_CONFIRMATORY_REPORT.md",
        rows,
        floor_rows,
        primary_rows,
        sk_rows,
